 ...
 *         If any slot is missing, kicks off background generation so the next
 *         poll will have more results. The client polls every 8s until complete.
 *
...
export async function GET() {
  ...
  // Kick off background generation for any missing slot
  for (const slot of SLOTS) {
    if (!cache[slot.id]) triggerGeneration(slot);
  }
